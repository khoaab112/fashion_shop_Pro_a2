    ...
    <div class="container-login100">
      <div class="wrap-login100">
        ...
        <form class="login100-form validate-form">
          <span class="login100-form-title h1"> QUẢN TRỊ </span>
          <div
          ...
          <div>
            <span v-if="error.username.err" class="error-message">{{
              error.username.content
            }}</span>
            <span v-if="error.password.err" class="error-message">{{
...
export default {
  mixins: [methodDefine, loginSuccess],
  name: "loginAdmin",
  components: {
    LoadingSpinner,
  },
  ...
  mounted() {
    ...
    Tilt.prototype.getValues = function (event) {
      ...
      var angle =
        Math.atan2(
          event.clientX - (this.left + this.width / 2),
          -(event.clientY - (this.top + this.height / 2))
        ) *
        (180 / Math.PI);
      return {
        tiltX: tiltX,
      ...
      if (!this.ticking) {
        ...
        if (this.settings.glare) {
          ...
          this.glareElement.style.opacity =
            (values.percentageY * this.settings.maxGlare) / 100;
        }
        ...
        this.element.style.transform =
          ...
          "px) rotateX(" +
          (this.settings.disableAxis === "x" ? 0 : values.tiltY) +
          "deg) rotateY(" +
          (this.settings.disableAxis === "y" ? 0 : values.tiltX) +
          "deg) scale3d(" +
          this.settings.scale +
          ", " +
          this.settings.scale +
    ...
    Tilt.prototype.reset = function () {
      this.element.style.transition =
        "transform " + this.settings.speed + "ms " + this.settings.easing;
      this.element.style.transform =
        "perspective(" +
        this.settings.perspective +
        "px) rotateX(0deg) rotateY(0deg) scale3d(" +
        this.settings.scale +
        ", " +
        this.settings.scale +
        ", " +
    ...
    clickLogin() {
      ...
      if (this.checkStrangeCharacters(this.password)) {
        ...
        this.error.password.content =
          "Chỉ chấp nhận mật khẩu chứa các chữ thưởng, in hoa, số, một vài ký tự (@,!,#,$,_,-)";
        return;
      }
        ...
        API.loginAdmin(user)
          .then((response) => {
            ...
            if (
              ...
              resultsReturned.result_code == 200
            ) {
              ...
              resultsReturned.status == "error" &&
              resultsReturned.result_code == 401
            ) {
              this.error.password.err = true;
